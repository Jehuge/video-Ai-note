...
4. 选择视频候选、清晰度、是否带截图、是否发送站点 cookies、笔记模式。
...
- 支持检测 DOM 视频地址、浏览器 performance media resources，以及 `.m3u8`、`.mpd`、`.mp4`、`.webm`、`.m4s`、`.ts` 等网络媒体请求。
- 对 `blob:` 视频会回退到把当前页面 URL 发送给本机 App，由后端 `yt-dlp` 继续解析。
- 只有用户勾选“站点 cookies”时才发送当前站点 cookies。
